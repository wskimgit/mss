...
Date: 2026-09-26
Parent: v6.1.1
Change type: MINOR — fresh-discovery contract; every run rebuilds candidate priority from the latest open market and forbids incumbent recommendation bias
...
# 7-A. FRESH DISCOVERY / NO INCUMBENT BIAS — MANDATORY
Every MSS execution MUST rebuild candidate priority from the latest validated open-market universe.

Rules:
- Previous final recommendations are NOT the starting candidate list.
- Previous SELECT NOW / STABILIZING / WATCH / WAIT states receive NO automatic priority, bonus, protection, or reserved slot.
- Previous rank, score, candidate order, and final recommendation must not be reused as current evidence.
- Reusable runtime assets may provide validated market data only; they must not carry forward recommendation priority.
- Every symbol, including a previously recommended symbol, must re-qualify through the current run's open-market funnel using current evidence.
- A previous recommendation may reappear only because it independently qualifies again under the latest data and AI review.
- Newly emerging symbols must be allowed to displace all previous recommendations.
- If the current market evidence changes materially, the final list should change without loyalty to the prior run.
- Historical recommendations remain available only for audit/comparison after the new candidate set has been formed; they are not an input to ranking.
- The AI Independent Review must explicitly check for incumbent/recommendation anchoring bias.

Fresh-discovery sequence:
LATEST VALIDATED OPEN MARKET
→ NEW CURRENT-RUN SCORING
→ NEW 300→100→30→10 COMPRESSION
→ CURRENT EVIDENCE REVIEW
→ AI FINAL
→ only then compare with the previous run for change reporting.

This rule does not require unnecessary redownload of unchanged raw market files. Valid, fresh raw/universe data may be reused after freshness validation, but candidate ranking and final recommendations are rebuilt for the current run.
...
Questions:
- Did the run start from the latest open-market universe rather than the previous recommendation list?
- Did any prior recommendation receive hidden priority, bonus, protection, or anchoring advantage?
- Were previous candidates used only after current-run selection for comparison/reporting?
...
# 16. MANDATORY LAST CONCLUSION BLOCK
Every MSS execution MUST end after Block 8 with the following visually separated conclusion block.
...
Rules:
- "MSS 한줄 결론" is the heading.
- Each field MUST appear on its own new line exactly in the order shown.
- Do not collapse the fields into one physical line.
- Preserve the vertical-bar prefix "│" for the four detail lines.
- No blank explanatory paragraph may be inserted inside the conclusion block.
- No text may appear after the final "│ 핵심:" line.
...
Restored:
- mandatory last conclusion block, displayed with one field per line.

# 18-A. INTENTIONAL v6.1.1 EXTENSION
Added:
- mandatory fresh discovery on every execution,
- no incumbent recommendation bias,
- prior recommendations cannot seed current ranking,
- prior recommendations may reappear only after current-run requalification,
- historical recommendations are comparison-only after current selection,
- reusable market data does not imply reusable recommendation priority.
...
69 conclusion block mandatory
70 conclusion fields must use line breaks
71 no text after final core line
...
77 fresh discovery every run
78 previous recommendations not starting list
79 no incumbent priority/bonus/protection
80 no prior rank/score reuse as evidence
81 runtime data reuse does not reuse recommendation priority
82 every prior recommendation must re-qualify
83 new symbols may displace all prior recommendations
84 prior recommendations comparison-only after current selection
85 independent review checks anchoring bias
86 current-run scoring rebuilt from latest validated universe
...
→ validate reusable raw/universe assets without inheriting prior recommendation priority
→ fresh current-run scoring from the latest open market
...
→ 8-block output
→ mandatory last conclusion block with line-broken fields.

No removed indicator or unavailable intraday step appears in the active execution path.


# v6.2.0 FINAL APPROVAL / FREEZE

Parent                                  │ v6.1.1
Change Type                             │ MINOR
Fresh Discovery Every Run               │ PASS
Previous Recommendation Seed Ban        │ PASS
Incumbent Priority Ban                  │ PASS
Current-Run Requalification             │ PASS
New Candidate Displacement              │ PASS
Anchoring-Bias Review                    │ PASS
Raw Asset Reuse Without Rank Reuse       │ PASS
Core Contract Inheritance               │ PASS
Regression                              │ PASS (86/86)
